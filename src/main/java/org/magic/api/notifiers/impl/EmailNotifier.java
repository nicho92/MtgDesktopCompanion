...
public class EmailNotifier extends AbstractMTGNotifier {
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		return Map.of("SMTP", new MTGProperty("smtp.server.com", "the smtp server name or ip"), "PORT",
				MTGProperty.newIntegerProperty("25", "the smtp port", 25, -1), "FROM",
				new MTGProperty("me@server.com", "the sender email."), "SEND_TO",
				new MTGProperty("you@server.com", "the destination mail for notification"), SMTP_LOGIN,
				new MTGProperty("login", "allowed user for the smtp server"), SMTP_PASS,
				new MTGProperty("password", "password for the authentication"), "SSL",
				MTGProperty.newBooleanProperty("true", "the smtp server use SSL"));
	}
