...
public class MondialRelayTrackingService extends AbstractTrackingService {
	...
	public Tracking track(String number, Contact c) throws IOException {
		...
		track.setTrackingUri("https://www.mondialrelay.fr/suivi-de-colis/?numeroExpedition=" + number + "&codePostal="
				+ c.getZipCode());
		track.setNumber(number);

		var ret = RequestBuilder.build()
				.url("https://www.mondialrelay.fr/_mvc/fr-FR/SuiviExpedition/RechercherJsonResponsive").newClient()
				.post().addContent("CodeMarque", "").addContent("NumeroExpedition", number)
				.addContent("CodePostal", c.getZipCode()).toJson().getAsJsonObject();

		if (!ret.get("Success").getAsBoolean()) {
			try {
				throw new IOException(URLTools.toHtml(
						(ret.get("ModelState").getAsJsonArray().get(0).getAsJsonObject().get("Item2").getAsString()))
						.text());
			} catch (Exception e) {
