...
public class DHLTrackingService extends AbstractTrackingService {
	...
	public Tracking track(String number, Contact c) throws IOException {
		...
		var res = RequestBuilder.build().newClient().url(BASEURL + number).get()
				...
				.addHeader(URLTools.ACCEPT, URLTools.HEADER_JSON).toJson().getAsJsonObject().get("shipments")
				.getAsJsonArray();
		...
		e.get("events").getAsJsonArray()
				.forEach(je -> t.addStep(
						new TrackingStep(UITools.parseGMTDate(je.getAsJsonObject().get("timestamp").getAsString()),
								je.getAsJsonObject().get("description").getAsString(),
								je.getAsJsonObject().get("statusCode").getAsString())));
