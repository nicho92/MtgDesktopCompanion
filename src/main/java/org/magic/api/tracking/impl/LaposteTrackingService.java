...
public class LaposteTrackingService extends AbstractTrackingService {
	...
	public Tracking track(String number, Contact c) throws IOException {
		...
		var e = RequestBuilder.build().newClient().url(baseUri + "/" + number + "?lang=" + getString("LANG")).get()
		...
		e.getAsJsonObject().get(SHIPMENT).getAsJsonObject().get("event").getAsJsonArray().forEach(je -> {
			t.addStep(new TrackingStep(UITools.parseGMTDate(je.getAsJsonObject().get("date").getAsString()),
					je.getAsJsonObject().get("label").getAsString(), je.getAsJsonObject().get("code").getAsString()));

			if (ArrayUtils.contains(new String[]{"DI1", "DI2", "DI0"},
					je.getAsJsonObject().get("code").getAsString())) {
