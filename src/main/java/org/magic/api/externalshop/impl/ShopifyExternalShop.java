...
public class ShopifyExternalShop extends AbstractExternalShop {
	...
	private Map<String, String> headers() {
		return Map.of(X_SHOPIFY_ACCESS_TOKEN, getAuthenticator().get(ACCESS_TOKEN), URLTools.ACCEPT,
				URLTools.HEADER_JSON, URLTools.CONTENT_TYPE, URLTools.HEADER_JSON);
	}

	private RequestBuilder build(String url, METHOD m) {
		return RequestBuilder.build().url(url).method(m).setClient(client).addHeaders(headers()).addContent("limit",
				"250");
	}
	...
	private JsonArray read(String entityName, Map<String, String> attributs) throws IOException {
		...
		arr.addAll(URLTools.toJson(resp.getEntity().getContent()).getAsJsonObject().get(entityName + "s")
				.getAsJsonArray());
	...
	private List<MTGStockItem> parseVariants(JsonObject obj) {
		...
		for (JsonElement el : obj.get("variants").getAsJsonArray()) {
			...
			try {
				it.getProduct()
						.setEdition(new MTGEdition(
								el.getAsJsonObject().get(OPTION + getString(SET_OPTION_NUMBER)).getAsString(),
								el.getAsJsonObject().get(OPTION + getString(SET_OPTION_NUMBER)).getAsString()));
			} catch (Exception e) {
			...
			it.setFoil(el.getAsJsonObject().get(OPTION + getString(FOIL_OPTION_NUMBER)).getAsString().toLowerCase()
					.contains("foil"));
			ret.add(it);
		}
		return ret;
	...
	public List<MTGProduct> listProducts(String name) throws IOException {
		return StreamSupport.stream(read(PRODUCT, null).spliterator(), true)
				...
				.map(a -> parseProduct(a.getAsJsonObject())).toList();
	}
	...
	protected void saveOrUpdateStock(List<MTGStockItem> it) throws IOException {
		...
		for (MTGStockItem c : it) {
			...
			if (c.getId() < 0) {
				...
				res = client.doPut(getBaseUrl() + "variants/" + c.getId() + JSON, new StringEntity(obj.toString()),
						headers());
