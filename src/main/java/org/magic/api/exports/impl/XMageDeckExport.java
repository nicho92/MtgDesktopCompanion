...
public class XMageDeckExport extends AbstractFormattedFileCardExport {
	...
	public void exportDeck(MTGDeck deck, File dest) throws IOException {
		...
		for (MTGCard mc : deck.getMain().keySet().stream().filter(mc -> mc != deck.getCommander())
				.sorted((mc, mc2) -> mc.getName().compareTo(mc2.getName())).toList()) {
			temp.append(deck.getMain().get(mc)).append(" ").append("[").append(mc.getEdition().getId()).append(":")
					.append(mc.getNumber()).append("]").append(" ").append(mc.getName()).append("\n");
			notify(mc);
		}
		for (MTGCard mc : deck.getSideBoard().keySet().stream()
				.sorted((mc, mc2) -> mc.getName().compareTo(mc2.getName())).toList()) {
			temp.append("SB: ").append(deck.getSideBoard().get(mc)).append(" ").append("[")
					.append(mc.getEdition().getId()).append(":").append(mc.getNumber()).append("]").append(" ")
					.append(mc.getName()).append("\n");
			notify(mc);
		}

		if (deck.getCommander() != null) {
			temp.append("SB: ").append("1").append(" ").append("[").append(deck.getCommander().getEdition().getId())
					.append(":").append(deck.getCommander().getNumber()).append("]").append(" ")
					.append(deck.getCommander().getName()).append("\n");
