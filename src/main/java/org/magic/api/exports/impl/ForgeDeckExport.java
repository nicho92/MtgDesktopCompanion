...
public class ForgeDeckExport extends AbstractFormattedFileCardExport {
	...
	public void exportDeck(MTGDeck deck, File dest) throws IOException {
		...
		if (deck.getCommander() != null) {
			...
			temp.append("1 ").append(deck.getCommander().getName()).append("|")
					.append(deck.getCommander().getEdition().getId().toUpperCase()).append("\n");
		}

		temp.append("[Main]\n");
		deck.getMain().entrySet().stream().filter(mc -> mc.getKey() != deck.getCommander())
				.sorted((e1, e2) -> e1.getKey().getName().compareTo(e2.getKey().getName()))
				.forEach(e -> temp.append(e.getValue()).append(" ").append(e.getKey().getName()).append("|")
						.append(e.getKey().getEdition().getId().toUpperCase()).append("|1\n"));

		temp.append("[Sideboard]\n");
		deck.getSideBoard().entrySet().stream()
				.sorted((e1, e2) -> e1.getKey().getName().compareTo(e2.getKey().getName()))
				.forEach(e -> temp.append(e.getValue()).append(" ").append(e.getKey().getName()).append("|")
						.append(e.getKey().getEdition().getId().toUpperCase()).append("|1\n"));
