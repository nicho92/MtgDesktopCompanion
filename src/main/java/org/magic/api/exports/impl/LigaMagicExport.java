...
public class LigaMagicExport extends AbstractCardExport {
	...
	public void exportStock(List<MTGCardStock> stock, File f) throws IOException {
		...
		try (Workbook workbook = new XSSFWorkbook(); var out = new FileOutputStream(f)) {
			...
			for (String s : new String[]{"Nome do Produto (PT)", "Nome do Produto (EN)", "Categoria", "Quantidade",
					"Preço", "Idioma", "Qualidade", "Foil (0, 1)", "Foil Etched(0, 1)", "Alterada (0, 1)",
			...
			for (MTGCardStock st : stock) {
				...
				row.createCell(colNum++, CellType.STRING)
						.setCellValue(st.getProduct().getForeignNames().stream()
								.filter(mcn -> mcn.getLanguage().contains("Portuguese")).findFirst()
								.orElse(st.getProduct().getForeignNames().get(0)).getName());
				...
				row.createCell(colNum++, CellType.STRING)
						.setCellValue(aliases.getConditionFor(this, st.getCondition()));
				...
				row.createCell(colNum++, CellType.NUMERIC)
						.setCellValue(st.getProduct().getPromotypes().contains(EnumPromoType.BUYABOX) ? 1 : 0);
				row.createCell(colNum++, CellType.NUMERIC).setCellValue(0);
				row.createCell(colNum++, CellType.NUMERIC)
						.setCellValue(st.getProduct().getPromotypes().contains(EnumPromoType.FNM) ? 1 : 0);
				row.createCell(colNum++, CellType.NUMERIC).setCellValue(st.getProduct().isOversized() ? 1 : 0);
				row.createCell(colNum++, CellType.NUMERIC).setCellValue(0);
				row.createCell(colNum++, CellType.NUMERIC)
						.setCellValue(st.getProduct().getEdition().getSet().length() == 4
								&& st.getProduct().getEdition().getSet().startsWith("P") ? 1 : 0);
