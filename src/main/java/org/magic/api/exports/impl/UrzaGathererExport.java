...
public class UrzaGathererExport extends AbstractFormattedFileCardExport {
	...
	public void exportDeck(MTGDeck deck, File dest) throws IOException {
		StringBuilder temp = new StringBuilder("\"sep=").append(getSeparator()).append("\"")
	...
	private MTGCard readCard(Matcher m) {
		try {
			return getEnabledPlugin(MTGCardsProvider.class).searchCardByName(m.group(1), null, true).stream()
					.filter(c -> (!m.group(18).isEmpty() && m.group(18).equals(c.getMultiverseid()))
							|| (m.group(15).equals(c.getNumber())))
	...
	private void writeDeckLine(StringBuilder temp, Set<Entry<MTGCard, Integer>> set, int i) {
		...
		set.forEach(entry -> {
			...
			temp.append(getSeparator()).append((i == 1) ? entry.getValue() : 0).append(getSeparator())
					.append((i == 2) ? entry.getValue() : 0).append(getSeparator())
					.append((i == 3) ? entry.getValue() : 0).append(System.lineSeparator());
		});
	...
	private void writeLine(StringBuilder temp, MTGCardStock mcs) {
		...
		temp.append("$")
				.append(!mcs.isFoil() ? UITools.formatDouble(mcs.getValue().doubleValue()).replace(",", ".") : 0)
				.append(getSeparator());
		temp.append("$").append(mcs.isFoil() ? UITools.formatDouble(mcs.getValue().doubleValue()).replace(",", ".") : 0)
				.append(getSeparator());
		...
		temp.append("\"").append(mcs.getQte()).append("x").append(aliases.getConditionFor(this, mcs.getCondition()))
