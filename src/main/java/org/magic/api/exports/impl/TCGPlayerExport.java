...
public class TCGPlayerExport extends AbstractFormattedFileCardExport {
	...
	public void exportStock(List<MTGCardStock> stocks, File f) throws IOException {
		...
		for (MTGCardStock mcs : stocks) {
			try {
				...
				temp.append(mcs.getProduct().isLand() ? "Land" : mcs.getProduct().getRarity().toPrettyString())
						.append(getSeparator());
				temp.append(mcs.getProduct().getTcgPlayerId()).append(getSeparator());
				temp.append("").append(getSeparator());
				temp.append("$")
						.append(mcs.getQte() * UITools.roundDouble(MTGControler.getInstance().getCurrencyService()
								.convert(MTGControler.getInstance().getCurrencyService().getCurrentCurrency(),
										Currency.getInstance("USD"), mcs.getValue().doubleValue())))
						.append(getSeparator());
				temp.append("$")
						.append(UITools.roundDouble(MTGControler.getInstance().getCurrencyService().convert(
	...
	public List<MTGCardStock> importStock(String content) throws IOException {
		...
		matches(content, true).forEach(m -> {
			...
			try {
				var mc = MTG.getEnabledPlugin(MTGCardsProvider.class).getCardByNumber(m.group(5),
						MTG.getEnabledPlugin(MTGCardsProvider.class).getSetById(aliases.getSetIdFor(this, m.group(6))));
				st.setProduct(mc);
				found = true;
			} catch (Exception e) {
			...
			if (!found) {
				try {
					var mc = MTG.getEnabledPlugin(MTGCardsProvider.class)
							.searchCardByName(m.group(3).replace("\"", ""), MTG.getEnabledPlugin(MTGCardsProvider.class)
									.getSetById(aliases.getSetIdFor(this, m.group(6))), true)
							.get(0);
					st.setProduct(mc);
					found = true;
				} catch (Exception e) {
					logger.error("no card found by name for {} for set {} : {}", m.group(3), m.group(6),
							e.getMessage());
				}
			}
