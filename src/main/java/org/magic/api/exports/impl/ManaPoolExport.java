...
public class ManaPoolExport extends AbstractCardExport {
	...
	public String getVersion() {
		return POMReader.readVersionFromPom(ManaPoolAPIService.class,
				"/META-INF/maven/com.github.nicho92/manapool-api-java/pom.properties");
	}
	...
	public void exportStock(List<MTGCardStock> stock, File f) throws IOException {
		...
		var items = stock.stream().map(mcs -> {
			...
			if (MTGControler.getInstance().getCurrencyService().isEnable())
				item.setPrice(MTGControler.getInstance().getCurrencyService().convertTo(Currency.getInstance("USD"),
						mcs.getValue().doubleValue()));
			else
	...
	private MTGCardStock filter(List<MTGCardStock> stock, Product p) {
		...
		mcs.getProduct().getScryfallId().equals(p.getScryfallId()) && (p.getFinishId() == EnumFinish.FO ? mcs.isFoil() : true) && (p.getCondition() == EnumCondition
						.valueOf(aliases.getConditionFor(this, mcs.getCondition(), p.getCondition().name())))
