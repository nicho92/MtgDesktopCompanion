...
public class PDFExport extends AbstractCardExport {
	...
	private Cell createCell(MTGCard card) throws IOException {
		...
		try {
			imageData = ImageDataFactory.create(getEnabledPlugin(MTGPictureProvider.class).getFullSizePicture(card),
					null);
		} catch (Exception _) {
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		return Map.of("AUTHOR", new MTGProperty(System.getProperty("user.name"), " the Author of the PDF File"),
				"PRINT_CUT_LINE", MTGProperty.newBooleanProperty("false", "mark the cutline between cards"), SPACE,
				MTGProperty.newIntegerProperty("0", "set padding between cards", 0, -1));
