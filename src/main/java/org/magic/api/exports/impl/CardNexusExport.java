...
public class CardNexusExport extends AbstractFormattedFileCardExport {
	...
	public void exportStock(List<MTGCardStock> stock, File f) throws IOException {
		...
		for (var mcs : stock) {
			...
			builder.append("\"").append(mcs.getProduct().getName().replace("\"", "")).append("\"")
					.append(getSeparator());
			...
			builder.append("\"").append(aliases.getConditionFor(this, mcs.getCondition())).append("\"")
					.append(getSeparator());
			builder.append("\"").append(mcs.getLanguage().substring(0, 2)).append("\"").append(getSeparator());
			builder.append("\"").append(UITools.formatDouble(mcs.getPrice(), '.')).append("\"")
