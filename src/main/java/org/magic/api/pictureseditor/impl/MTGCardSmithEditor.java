...
public class MTGCardSmithEditor extends AbstractPicturesEditorProvider {
	...
	private boolean isNormalLayout(MTGCard mc) {
		return !mc.isBorderLess() && !mc.getFrameEffects().contains(EnumFrameEffects.ETCHED) && !mc.isVehicule()
				&& !mc.getFrameEffects().contains(EnumFrameEffects.ENCHANTMENT) && !mc.isSnow()
				&& !mc.getFrameEffects().contains(EnumFrameEffects.DEVOID);
	}
	...
	public BufferedImage getPicture(MTGCard mc, MTGEdition me) throws IOException {
		...
		var build = RequestBuilder.build().url(urlBuilder + "?fromAjax=1&v=3").setClient(client).post()
				.addHeader("x-requested-with", "XMLHttpRequest").addHeader(URLTools.REFERER, BASE_URL)
				.addHeader(URLTools.HOST, "devm.mtgcardsmith.com").addHeader(URLTools.ORIGIN, BASE_URL)
				...
				.addContent("pos_sub_x", "").addContent("pos_sub_y", "")
				.addContent("frame_name", EnumColors.determine(mc.getColors()).toPrettyString())
				.addContent("pos_cost_y", "").addContent("pos_desc_f", "Mplantin.ttf").addContent("pos_desc_x", "")
				.addContent("pos_desc_y", "").addContent("pos_title_f", "beleren-bold_P1.01.ttf")
				.addContent("pos_title_x", "").addContent("pos_title_y", "").addContent("pos_bottom_x", "")
				.addContent("pos_bottom_y", "").addContent("pos_rarity_x", "").addContent("pos_rarity_y", "")
				.addContent("pos_bottom2_x", "").addContent("pos_bottom2_y", "").addContent("pos_bottom3_x", "")
				.addContent("pos_bottom3_y", "").addContent("frame_category", "Standard Colors")
				.addContent("name", mc.getName())
				.addContent("title_color",
						mc.getCustomMetadata().getOrDefault(EnumExtraCardMetaData.TEXT_COLOR, BLACK_COLOR))
				.addContent("custom_mana", getCost(mc.getCost())).addContent("watermark", mc.getWatermarks())
				...
				.addContent("subtype_color",
						mc.getCustomMetadata().getOrDefault(EnumExtraCardMetaData.TEXT_COLOR, BLACK_COLOR))
				.addContent("type",
						mc.getSupertypes().stream().collect(Collectors.joining(" ")) + " "
								+ mc.getTypes().stream().collect(Collectors.joining(" ")))
				.addContent("custom_type", "")
				.addContent("subtype", mc.getSubtypes().stream().collect(Collectors.joining(" ")))
				.addContent("rarity", mc.getRarity().toPrettyString().toLowerCase()).addContent("set_icon", "mtgcs1")
				.addContent("body_color",
						mc.getCustomMetadata().getOrDefault(EnumExtraCardMetaData.TEXT_COLOR, BLACK_COLOR))
				.addContent("text_size",
						(size == 18 ? "vsmall" : size <= 20 ? "small" : size <= 24 ? "large" : "vlarge"))
				.addContent("description",
						minimize(mc.getText()) + (mc.getFlavor().isEmpty() ? "" : "\n<i>" + mc.getFlavor() + "</i>"))
				.addContent("power", mc.getPower()).addContent("toughness", mc.getToughness())
				.addContent("pos_pt_tx", "0").addContent("pos_pt_ty", "0").addContent("pt_color", BLACK_COLOR)
				.addContent("pos_pt_x", "0").addContent("pos_pt_y", "0").addContent("artist", mc.getArtist())
				.addContent("artist_color", "#ffffff").addContent("category", "")
				.addContent("creator", getString("DESIGNER")).addContent("set", mc.getEdition().getId())
		...
		if (mc.isVehicule()) {
			...
			build.addContent("special_card_color",
					"vehart01" + (c == EnumColors.GOLD ? "m" : c.getCode().toLowerCase()));
		}
		...
		if (mc.isLegendary() && isNormalLayout(mc)) {
			...
			build.addContent("special_card_color", "lg"
					+ (color == EnumColors.GOLD || color == EnumColors.UNCOLOR ? "o" : color.getCode().toLowerCase()));
		...
		if (mc.isHybride()) {
			build.removeContent("frame");
			build.addContent("special_card_color",
					mc.getCustomMetadata().get(EnumExtraCardMetaData.ACCENT).toLowerCase());
		}
		...
		return URLTools.extractAsImage(BASE_URL
				+ URLTools.toJson(res.getEntity().getContent()).getAsJsonObject().get("success").getAsString());
	}
	...
	private String getCost(String cost) {
		...
		if (cost.contains("/P")) {
			cost = cost.replace("C/P", "P/C").replace("W/P", "P/W").replace("U/P", "P/U").replace("B/P", "P/B")
					.replace("R/P", "P/R").replace("G/P", "P/G");
		}
	...
	private String minimize(String text) {
		...
		return Pattern.compile(EnumCardsPatterns.MANA_PATTERN.getPattern()).matcher(text)
	...
	private void connect() throws IOException {
		...
		var nonce = RequestBuilder.build().url(urlAuthentication).setClient(client).get().toHtml()
				.getElementById("woocommerce-login-nonce").attr("value");

		RequestBuilder.build().url(urlAuthentication).setClient(client).post().addHeader(URLTools.ACCEPT,
				"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7")
				.addHeader(URLTools.ACCEPT_ENCODING, "gzip, deflate, br, zstd")
				.addHeader(URLTools.CONTENT_TYPE, "application/x-www-form-urlencoded").addHeader("Pragma", "no-cache")
				.addHeader("Connection", "keep-alive").addHeader(URLTools.HOST, "mtgcardsmith.com")
				.addHeader(URLTools.ORIGIN, BASE_URL).addHeader(URLTools.REFERER, urlAuthentication)
				...
				.addContent("password", getAuthenticator().getPassword()).addContent("_wp_http_referer", "/my-account/")
				.addContent("login", "Log in").addContent("rememberme", "forever")
				.addContent("woocommerce-login-nonce", nonce)

				.execute();

		try {
			var c = RequestBuilder.build().clean().url(urlAuthentication).setClient(client).get().toHtml()
					.select("div.woocommerce-MyAccount-content p strong").first().text();
	...
	private String uploadPicture(File f, MTGCard mc) throws IOException {
		var res = RequestBuilder.build().url(urlPictureUpload).setClient(client).post().addHeader(URLTools.ACCEPT,
				"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7")
				.addHeader(URLTools.ACCEPT_ENCODING, "gzip, deflate, br, zstd")
				.addHeader(URLTools.CONTENT_TYPE, "application/x-www-form-urlencoded").addHeader("Pragma", "no-cache")
				.addHeader("Connection", "keep-alive").addHeader(URLTools.REFERER, BASE_URL + "/mtg-card-maker")
				.addHeader(URLTools.HOST, "mtgcardsmith.com").addHeader(URLTools.ORIGIN, BASE_URL)
				.addContent("location", "mtg-card-maker").addContent("status", "new")
				.addContent("slim[]", generateJsonData(f, mc.getCustomMetadata()).toString()).execute();

		logger.info("{} results {}", urlPictureUpload, res.getStatusLine());

		var doc = RequestBuilder.build().url(BASE_URL + "/mtg-card-maker/edit").setClient(client).get().addHeader(
				URLTools.ACCEPT,
				"text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7")
				.addHeader(URLTools.ACCEPT_ENCODING, "gzip, deflate, br, zstd")
				.addHeader(URLTools.ACCEPT_LANGUAGE, "fr-FR,fr;q=0.9,en;q=0.8").addHeader("cache-control", "no-cache")
				.addHeader(URLTools.REFERER, BASE_URL + "/mtg-card-maker").addHeader("pragma", "no-cache")
	...
	private String uploadPicture2(File f, MTGCard mc) throws IOException {
		...
		var res = client.doPost(BASE_URL + "/api/imgUpload.php",
				MultipartEntityBuilder.create().addPart("file", new FileBody(f)).build(), hs);
	...
	private JsonObject generateJsonData(File f, Map<EnumExtraCardMetaData, String> map) throws IOException {
		...
		content.get("output").getAsJsonObject().addProperty("width",
				map.getOrDefault(EnumExtraCardMetaData.CROP_W, "" + img.getWidth()));
		content.get("output").getAsJsonObject().addProperty("height",
				map.getOrDefault(EnumExtraCardMetaData.CROP_H, "" + img.getHeight()));
		content.get("output").getAsJsonObject().addProperty("image",
				"data:image/png;base64," + CryptoUtils.toBase64(binary));

		content.get("actions").getAsJsonObject().get("crop").getAsJsonObject().addProperty("x",
				map.getOrDefault(EnumExtraCardMetaData.CROP_X, "0"));
		content.get("actions").getAsJsonObject().get("crop").getAsJsonObject().addProperty("y",
				map.getOrDefault(EnumExtraCardMetaData.CROP_Y, "0"));
		content.get("actions").getAsJsonObject().get("crop").getAsJsonObject().addProperty("width",
				map.getOrDefault(EnumExtraCardMetaData.CROP_W, "" + img.getWidth()));
		content.get("actions").getAsJsonObject().get("crop").getAsJsonObject().addProperty("height",
				map.getOrDefault(EnumExtraCardMetaData.CROP_H, "" + img.getHeight()));
