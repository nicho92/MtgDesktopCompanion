...
public class H2DAO extends AbstractMagicSQLDAO {
	...
	protected String getjdbcUrl() {
		return "jdbc:" + getjdbcnamedb() + (getString(MODE).equals("file") ? "/" : ":") + getString(DB_NAME)
				+ ";CASE_INSENSITIVE_IDENTIFIERS=TRUE" + (getString(MODE).equals("mem") ? ";DB_CLOSE_DELAY=-1" : "");
	}
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		...
		m.get(SERVERNAME)
				.setDefaultValue(Paths.get(MTGConstants.DATA_DIR.getAbsolutePath()).toFile().getAbsolutePath());
