...
public class WallhavenWallpaperProvider extends AbstractJsonWallpaperProvider {
	...
	protected List<MTGWallpaper> parse(JsonObject obj) {
		...
		wall.setFormat(
				obj.get("file_type").getAsString().substring(obj.get("file_type").getAsString().indexOf("/") + 1));
	...
	protected RequestBuilder createQuery(String search, int page) {
		...
		return RequestBuilder.build().get().newClient().url("https://wallhaven.cc/api/v1/search")
				...
				.addContent("categories", catBuilder.toString()).addContent("purity", pBuilder.toString())
