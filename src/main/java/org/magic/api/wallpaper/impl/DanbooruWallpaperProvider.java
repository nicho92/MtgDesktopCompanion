...
public class DanbooruWallpaperProvider extends AbstractJsonWallpaperProvider {
	...
	protected RequestBuilder createQuery(String search, int page) {
		return RequestBuilder.build().get().newClient().url("https://danbooru.donmai.us/posts.json")
				...
				.addContent("login", getAuthenticator().get("USER_ID")).addContent("tags", search.replace(" ", "_"))
