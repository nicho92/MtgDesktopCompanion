...
public class PhilibertShopper extends AbstractMagicShopper {
	...
	public Transaction getTransaction(RetrievableTransaction rt) throws IOException {
		...
		var orderPage = RequestBuilder.build().url(rt.getUrl()).setClient(client).get()
				.addHeader("x-requested-with", "XMLHttpRequest").toHtml();
		...
		for (Element tr : orderPage.select("div.detail-table-row")) {
			...
			stock.setPrice(UITools.parseDouble(URLTools
					.extractElementText(tr.select("div.detail-table-row__price div.detail-table-row__value").first())));
	...
	private void init() {
		...
		if (client == null) {
			...
			try {
				s = RequestBuilder.build().post().url(BASE_URL + "/en/authentication").setClient(client)
						.addContent("email", getAuthenticator().getLogin())
						.addContent("passwd", getAuthenticator().getPassword()).addContent("back", "history")
						.addContent("SubmitLogin", "").addHeader(":path", "/en/authentication")
						.addHeader(":authority", "www.philibertnet.com").addHeader(":method", METHOD.POST.name())
						.addHeader(URLTools.CONTENT_TYPE, "application/x-www-form-urlencoded").toHtml();
