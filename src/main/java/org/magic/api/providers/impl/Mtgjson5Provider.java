...
public class Mtgjson5Provider extends AbstractMTGJsonProvider {
	...
	public List<MTGCard> searchCardByCriteria(String att, String crit, MTGEdition ed, boolean exact)
		...
		String jsquery = ROOT_DATA + filterEdition + CARDS_ROOT_SEARCH + att + " =~ /^.*" + crit.replace("\\+", " ")
				+ ".*$/i)]";
	...
	private List<MTGCard> search(String jsquery) {
		...
		for (Map<String, Object> map : cardsElement) {
			...
			if (map.get(IS_GAME_CHANGER) != null)
				mc.setGameChanger(Boolean.valueOf(map.get(IS_GAME_CHANGER).toString()));
			...
			if (map.get(KEYWORDS) != null)
				mc.getKeywords().addAll(((List<String>) map.get(KEYWORDS)).stream()
						.map(k -> new MTGKeyWord(k, MTGKeyWord.TYPE.ABILITIES)).toList());

			if (map.get(ATTRACTION_LIGHTS) != null)
				mc.setAttractionLights(
						((List<String>) map.get(KEYWORDS)).stream().mapToInt(Integer::parseInt).boxed().toList());

			if (map.get(LEGALITIES) != null) {

				for (Map.Entry<String, String> mapFormats : ((Map<String, String>) map.get(LEGALITIES)).entrySet()) {
					var mf = new MTGFormat(String.valueOf(mapFormats.getKey()),
							AUTHORIZATION.valueOf(String.valueOf(mapFormats.getValue()).toUpperCase()));
	...
	private void initOtherEditionCardsVar(MTGCard mc, MTGEdition me) {
		...
		String jsquery = "$.data." + edCode + ".cards[?(@.name==\""
	...
	private EnumExtra parseExtra(String extra) {
		...
		switch (extra) {
			case "draft" :
				return EnumExtra.DRAFT;
			case "set" :
				return EnumExtra.SET;
			case "collector" :
				return EnumExtra.COLLECTOR;
			case "vip" :
				return EnumExtra.VIP;
			case "jump" :
				return EnumExtra.JUMP;
			case "theme" :
				return EnumExtra.THEME;
	...
	private EnumItems parseType(String category) {
		switch (category) {
			case "case" :
				return EnumItems.CASE;
			case "subset" :
				return EnumItems.CASE;
			case "deck_box" :
				return EnumItems.CASE;
			case "booster_box" :
				return EnumItems.BOX;
			case "booster_pack" :
				return EnumItems.BOOSTER;
			case "bundle" :
				return EnumItems.BUNDLE;
			case "prerelease_pack" :
				return EnumItems.PRERELEASEPACK;
			case "deck" :
				return EnumItems.DECK;
			case "commander_deck" :
				return EnumItems.COMMANDER_DECK;
			case "draft_set" :
				return EnumItems.DRAFT_PACK;
			default :
				return null;
		}
	}
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		...
		m.put("LRU_CACHE",
				MTGProperty.newIntegerProperty("400", "Size of the LRU cache. Used for cache optimization", 50, -1));
		return m;
	}
