...
public class MarkovGenerator extends AbstractMTGTextGenerator {
		...
		if (!cache.exists() || cache.length() == 0) {
			...
			for (var mc : getEnabledPlugin(MTGCardsIndexer.class).listCards()) {
				...
				if ((mc.getText() != null || !mc.getText().isEmpty() || !mc.getText().equalsIgnoreCase("null"))) {
					var r = mc.getText().replace(EnumCardsPatterns.REMINDER.getPattern(), "").replace("\n", " ")
							.replace(mc.getName(), getString("TAG_NAME")).trim();
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		return Map.of("CACHE_FILE", MTGProperty.newFileProperty(new File(MTGConstants.DATA_DIR, "markov.gen")), "NGEN",
				MTGProperty.newIntegerProperty("5", "n-factor - the length of each n-gram stored in the model", 2, 10),
