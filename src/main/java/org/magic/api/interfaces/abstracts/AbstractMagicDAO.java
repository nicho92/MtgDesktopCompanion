...
public abstract class AbstractMagicDAO extends AbstractMTGPlugin implements MTGDao {
	...
	public List<MTGCard> listCardsFromCollection(MTGCollection collection, MTGEdition me) throws SQLException {
		return listStocks(collection, me).stream().map(MTGCardStock::getProduct).distinct()
	...
	public List<MTGAnnounce> listAnnounces(String textSearch) throws SQLException {
		return listAnnounces().stream().filter(a -> a.getTitle().toLowerCase().contains(textSearch)
				|| a.getDescription().toLowerCase().contains(textSearch)).toList();
	}
	...
	public List<MTGSealedStock> listSealedStocks(MTGCollection c) throws SQLException {
		return listSealedStocks().stream().filter(ss -> ss.getMagicCollection().getName().equalsIgnoreCase(c.getName()))
				.toList();
	}
	...
	public List<MTGSealedStock> listSealedStocks(MTGCollection c, MTGEdition ed) throws SQLException {
		return listSealedStocks().stream().filter(ss -> ss.getMagicCollection().getName().equalsIgnoreCase(c.getName())
				&& ss.getProduct().getEdition().getId().equalsIgnoreCase(ed.getId())).toList();
	}
