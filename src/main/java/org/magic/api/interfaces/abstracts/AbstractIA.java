...
public abstract class AbstractIA extends AbstractMTGPlugin implements MTGIA {
	...
	private JsonSchemaElement getCardElement() {
		return JsonObjectSchema.builder().addStringProperty(NAME)
				...
				.addStringProperty(COST).addIntegerProperty(CMC)
				.addEnumProperty(RARITY, List.of(EnumRarity.values()).stream().map(en -> en.getName()).toList())
				.addStringProperty(FLAVOR).addStringProperty(TEXT).addIntegerProperty(POWER)
				.addIntegerProperty(TOUGHNESS).addIntegerProperty(LOYALTY)
				.required(NAME, TYPES, COST, TEXT, FLAVOR, RARITY, CMC).build();
	}

	private ResponseFormat getGeneratedCardFormat() {
		return ResponseFormat.builder().type(ResponseFormatType.JSON)
				.jsonSchema(JsonSchema.builder().name("MTG Card").rootElement(getCardElement()).build()).build();
	}

	private ResponseFormat getGeneratedSetFormat() {
		return ResponseFormat.builder().type(ResponseFormatType.JSON)
				.jsonSchema(JsonSchema.builder().name("Set").rootElement(JsonObjectSchema.builder()
						.addProperty("cards", JsonArraySchema.builder().items(getCardElement()).build()).build())
						.build())
				.build();
	}
	...
	private ResponseFormat getGeneratedDeckFormat() {
		return ResponseFormat
				...
				.jsonSchema(
						JsonSchema.builder().name("MTG Deck")
								.rootElement(
										JsonObjectSchema.builder().addStringProperty(NAME)
												.addProperty("mainboard", JsonArraySchema.builder()
														.items(JsonObjectSchema.builder().addStringProperty(NAME)
																.addIntegerProperty(QUANTITY).build())
														.build())
												.addProperty("sideboard", JsonArraySchema.builder()
														.items(JsonObjectSchema.builder().addStringProperty(NAME)
																.addIntegerProperty(QUANTITY).build())
														.build())
												.addStringProperty("description").required("description", "mainboard")
												.build())
								.build())
				.build();
	}
	...
	public MTGCard generateRandomCard(String description, MTGEdition set, String number) throws IOException {
		...
		var obj = URLTools
				.toJson(getEngine(getGeneratedCardFormat()).chat(NEW_CARD_QUERY
						+ ((description == null || description.isEmpty()) ? "" : WITH_THIS_DESCRIPTION + description)))
				.getAsJsonObject();
	...
	public MTGDeck generateDeck(String description) throws IOException {
		var obj = URLTools
				.toJson(getEngine(getGeneratedDeckFormat()).chat(NEW_DECK_QUERY
						+ ((description == null || description.isEmpty()) ? "" : WITH_THIS_DESCRIPTION + description)))
				.getAsJsonObject();
		...
		obj.get("mainboard").getAsJsonArray().forEach(je -> {
			...
			try {
				var mc = MTG.getEnabledPlugin(MTGCardsProvider.class)
						.searchCardByName(je.getAsJsonObject().get(NAME).getAsString(), null, true).get(0);
	...
	public List<MTGCard> generateSet(String description, MTGEdition set, int qty) throws IOException {
		var obj = URLTools
				.toJson(getEngine(getGeneratedSetFormat()).chat(NEW_SET_QUERY.replace("X", "" + qty)
						+ ((description == null || description.isEmpty()) ? "" : WITH_THIS_DESCRIPTION + description)))
				.getAsJsonObject();
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		...
		map.put("TEMPERATURE", MTGProperty.newIntegerProperty("0.7",
				"You can think of temperature like randomness, with low value is being least random (or most deterministic) and max being most random (least deterministic)",
				0, 1));
