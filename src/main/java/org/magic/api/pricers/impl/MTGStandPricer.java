...
public class MTGStandPricer extends AbstractPricesProvider {
	...
	protected List<MTGPrice> getLocalePrice(MTGCard card) throws IOException {
		...
		var j = RequestBuilder.build().setClient(c).get().url(BASE_URL + "/data-singlecard.php")
				.addContent("job", "getsinglecard").addContent("sid", card.getScryfallId())
				.addContent("userCurrencyBuyer", cur).addContent("useridbuyer", "ZmxNd2tjSzFiNzB4bWN3V2M2TmVWUT09")
		...
		for (JsonElement el : arr) {
			...
			p.setQuality(aliases.getReversedConditionFor(this,
					URLTools.toHtml(obj.get("cond").getAsString()).select("span").text().replace("c:", ""),
					EnumCondition.NEAR_MINT));
			p.setValue(UITools.parseDouble(
					obj.get("price").getAsString().substring(0, obj.get("price").getAsString().indexOf(" "))));
