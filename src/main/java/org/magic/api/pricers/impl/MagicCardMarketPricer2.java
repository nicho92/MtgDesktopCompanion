...
public class MagicCardMarketPricer2 extends AbstractPricesProvider {
	...
	public static Product getProductFromCard(MTGCard mc, List<Product> list) {
		...
		for (Product p : list) {
			...
			if (p.getCategoryName().equalsIgnoreCase("Magic Single") && (p.getExpansionName().equalsIgnoreCase(edName)
					|| (p.getIdProduct() == mkmId && !mc.isExtraCard()))) {
				resultat = p;
				break;
			}
		}
		...
		try {
			...
			if (!getBoolean("USER_ARTICLE")) {
				...
				if (p != null) {
					...
					mp.setSellerUrl(
							MkmConstants.MKM_SITE_URL + "/fr/Magic/Users/" + mp.getSeller() + "/Offers/Singles");
					mp.setSite(getName());
					mp.setFoil(false);
					mp.setCurrency("EUR");
				...
				for (Article a : articles) {
					...
					if (StringUtils.isEmpty(getString(FILTER_COUNTRY))
							|| ArrayUtils.contains(getArray(FILTER_COUNTRY), mp.getCountry().toUpperCase()))
						lists.add(mp);
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		...
		map.put(LANGUAGE_ID, MTGProperty.newIntegerProperty("1",
		...
		map.put(MIN_CONDITION, new MTGProperty(
				"Minimum condition for filter.  (MT for Mint > NM for Near Mint > EX for Exellent > GD for Good > LP for Light Played > PL for Played > PO for Poor). Separated by comma",
				"", "MT", "NM", "EX", "EX", "GD", "LP", "PL", "PO"));
		map.put("COMMONCHECK",
				MTGProperty.newBooleanProperty(FALSE, "set to true if you want to check price of common rarity cards"));
		map.put("MAX", MTGProperty.newIntegerProperty("10", "Max result to return", 1, 20));
		map.put("USER_ARTICLE", MTGProperty.newBooleanProperty(TRUE,
				"Set to true if you want to check prices from users stock. False will return average"));
		map.put("AUTOMATIC_ADD_CARD_ALERT", MTGProperty.newBooleanProperty(FALSE,
				"set to true if you want to automaticaly add product to your online cart when card is found"));
		map.put(FILTER_COUNTRY, new MTGProperty("EN," + Locale.getDefault().getCountry(),
				" Filter users country for results. Separated by comma"));
