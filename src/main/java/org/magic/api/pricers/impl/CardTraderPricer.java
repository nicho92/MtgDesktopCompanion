...
public class CardTraderPricer extends AbstractPricesProvider {
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		...
		m.put("AUTOMATIC_ADD_CART",
				MTGProperty.newBooleanProperty("false", "set to true if you want add finded item to your online cart"));
		m.put(COUNTRY_FILTER, new MTGProperty("", "Set seller's country filter in card search. Separated by comma",
				"IT", "FR", "EN", "SK", "ES", "BE", "AT", "GR", "SE", "CH"));
	...
	protected List<MTGPrice> getLocalePrice(MTGCard card) throws IOException {
		...
		service.listMarketProductByBluePrint(bp).forEach(marketItem -> {
			...
			if (ArrayUtils.contains(getArray(COUNTRY_FILTER), marketItem.getSeller().getCountryCode())
					|| getString(COUNTRY_FILTER).isEmpty()) {
				...
				mp.setSite(getName());
				mp.setQuality(aliases.getReversedConditionFor(this, marketItem.getCondition().name(),
						EnumCondition.NEAR_MINT));
				mp.setSellerUrl(
						CardTraderConstants.CARDTRADER_WEBSITE_URI + "/users/" + marketItem.getSeller().getUsername());
				mp.setUrl(CardTraderConstants.CARDTRADER_WEBSITE_URI + "/cards/" + bp.getSlug() + "?share_code="
						+ CardTraderConstants.SHARE_CODE);
