...
public class MTGMintCardsPricer extends AbstractPricesProvider {
	...
	protected List<MTGPrice> getLocalePrice(MTGCard card) throws IOException {
		...
		var category = (card.isBorderLess() || card.isExtendedArt() || card.isShowCase() || card.isRetro())
				? "15"
				: "11";

		logger.info("{} looking for price for {}", getName(), card);

		var trs = RequestBuilder.build().url(URL_BASE + "mtg/singles/search").addContent("action", "normal_search")
				.addContent("keywords", card.getName()).addContent("fil_ml", category).setClient(client).get().toHtml()
				.select("tr#product_list_content");

		parse(card, ret, trs, false);

		// 12 is for regular foil, 16 for variants foil
		if (card.getFinishes().size() > 1) {
			trs = RequestBuilder.build().url(URL_BASE + "mtg/singles/search").addContent("action", "normal_search")
					.addContent("keywords", card.getName()).addContent("fil_ml", category.equals("11") ? "12" : "16")
					.setClient(client).get().toHtml().select("tr#product_list_content");
		...
		for (var tr : trs) {
			...
			if (tr.select("td a.opacityit").attr("href").contains(card.getEdition().getId().toLowerCase())) {
				...
				for (int i = 0; i < elementsQuality.size(); i++) {
					...
					p.setQuality(aliases.getReversedConditionFor(this, elementsQuality.get(i).text(),
