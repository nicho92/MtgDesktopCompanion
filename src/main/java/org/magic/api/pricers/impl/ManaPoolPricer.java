...
public class ManaPoolPricer extends AbstractPricesProvider {
	...
	protected List<MTGPrice> getLocalePrice(MTGCard card) throws IOException {
		...
		poolResults.stream().filter(pv -> pv.getScryfallId().equals(card.getScryfallId()) && pv.getAvailable() > 0)
				.forEach(pv -> {
					var mp = new MTGPrice();
					mp.setCardData(card);
					mp.setCurrency("USD");
					...
					mp.setSite(getName());
					mp.setUrl(pv.getUrl() + "?referrer_code=MP-unt564tzvR24HgRRfJk5EU&conditions="
							+ pv.getConditionId().name() + (mp.isFoil() ? "&finish=foil" : ""));
					...
					mp.setQuality(aliases.getReversedConditionFor(this, pv.getConditionId().name(),
							org.magic.api.beans.enums.EnumCondition.NEAR_MINT));
