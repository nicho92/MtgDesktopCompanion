...
public class EbayPricer extends AbstractPricesProvider {
	...
	public List<MTGPrice> getLocalePrice(MTGCard card) throws IOException {
		...
		var token = RequestBuilder.build().newClient().url(domain + "/identity/v1/oauth2/token")
				.addHeader(URLTools.CONTENT_TYPE, "application/x-www-form-urlencoded")
				.addHeader(URLTools.AUTHORIZATION,
						"Basic " + CryptoUtils.toBase64((clientId + ":" + clientSecret).getBytes()))
				.addContent("grant_type", "client_credentials").addContent("scope", domain + "/oauth/api_scope").post()
				.toJson().getAsJsonObject().get("access_token").getAsString();

		var query = RequestBuilder.build().newClient().url(domain + "/buy/browse/v1/item_summary/search")
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		return Map.of("MAX", MTGProperty.newIntegerProperty("10", "max results to return", 5, -1), "EBAY_MARKETPLACE",
				new MTGProperty("EBAY_US", "choose country market place. Type EBAY_XXX where is is the country code",
				...
				"FIXEDPRICE_ONLY",
				MTGProperty.newBooleanProperty("false", "chose if you want to filter to fixed price items"),
