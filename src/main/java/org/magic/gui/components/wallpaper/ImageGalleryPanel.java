...
public class ImageGalleryPanel extends MTGUIComponent {
			...
			protected void process(List<MTGWallpaper> chunks) {
				for (var img : chunks) {
					...
					try {
						...
						thumb.setIcon(
								new ImageIcon(ImageTools.resize(img.getPicture(), THUMBNAIL_SIZE, THUMBNAIL_SIZE)));
	...
	private void showFullImage(MTGWallpaper wall) {
		...
		var diag = MTGUIComponent.createJDialog(
				MTGUIComponent.build(container, wall.getName() + " By " + wall.getAuthor(), getIcon()), true, false);
		...
		ThreadManager.getInstance().invokeLater(new MTGRunnable() {
			...
			protected void auditedRun() {
				...
				try {
					...
					var b = RequestBuilder.build().setClient(client).get().url(wall.getUrl().toASCIIString()).addHeader(
							URLTools.ACCEPT,
