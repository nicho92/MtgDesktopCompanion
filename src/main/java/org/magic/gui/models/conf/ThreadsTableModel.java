...
public class ThreadsTableModel extends GenericTableModel<ThreadInfo> {
	...
	public ThreadsTableModel() {
		setColumns("ID", "PROCESS", "CPU (s.)", "STATE", "PRIORITY", "LOCK INFO", "BLOCKED COUNT", "DEADLOCKED",
				"MEMORY (KB.)");
	}
	...
	public Object getValueAt(int row, int column) {
		...
		try {
			switch (column) {
				...
				case 7 :
					return Arrays.asList(ManagementFactory.getThreadMXBean().findDeadlockedThreads())
							.contains(t.getThreadId());
				case 8 :
					return ((com.sun.management.ThreadMXBean) ManagementFactory.getThreadMXBean())
							.getThreadAllocatedBytes(t.getThreadId()) / 1024;
				default :
					return null;
			}
		} catch (Exception e) {
