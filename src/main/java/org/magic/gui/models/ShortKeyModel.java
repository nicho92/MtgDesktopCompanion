...
public class ShortKeyModel extends GenericTableModel<JButton> {
	...
	public ShortKeyModel() {
		setColumns("Module", "Button", "Alt + key");
	...
	public Object getValueAt(int row, int column) {
		switch (column) {
			case 0 :
				return SwingUtilities.getAncestorOfClass(MTGUIComponent.class, items.get(row));
			case 1 :
				return items.get(row);
			case 2 :
				return KeyEvent.getKeyText(items.get(row).getMnemonic());
			default :
