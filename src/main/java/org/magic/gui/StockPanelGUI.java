...
public class StockPanelGUI extends MTGUIComponent {
	...
	private String[] selections = new String[]{"", MTGControler.getInstance().getLangService().get("NEW"),
			...
			MTGControler.getInstance().getLangService().get("ALL")};
	...
	public StockPanelGUI() {
		...
		btnSave.addActionListener(_ -> {
			...
			var sw = new AbstractObservableWorker<Void, MTGCardStock, MTGDao>(lblLoading,
					getEnabledPlugin(MTGDao.class), updates.size()) {
				@Override
				protected void done() {
					super.done();
		...
		btnMerge.addActionListener(_ -> {
			...
			if (lines.stream().map(MTGCardStock::hashId).distinct().count() > 1) {
				MTGControler.getInstance()
						.notify(new MTGNotification("Error", "Stock items are not equals", MESSAGE_TYPE.ERROR));
				return;
			}
			...
			var sw = new AbstractObservableWorker<Void, MTGCardStock, MTGDao>(lblLoading,
					getEnabledPlugin(MTGDao.class), lines.size()) {
		...
		btnDelete.addActionListener(_ -> {
			...
			if (res == JOptionPane.YES_OPTION) {
				...
				var sw = new AbstractObservableWorker<Void, MTGCardStock, MTGDao>(lblLoading,
						getEnabledPlugin(MTGDao.class), stocks.size()) {
					@Override
					protected Void doInBackground() {
		...
		btnReload.addActionListener(_ -> {
			...
			if (model.getItems().stream().anyMatch(mcs -> mcs.isUpdated())) {
				int res = JOptionPane.showConfirmDialog(null, capitalize("CANCEL_CHANGES"), capitalize("CONFIRM_UNDO"),
		...
		btnImport.addActionListener(ae -> {
			...
			var mnuImportSearch = new JMenuItem(
					capitalize("IMPORT_FROM", MTGControler.getInstance().getLangService().get("SEARCH_MODULE")));
				...
				if (exp.getMods() == MODS.BOTH || exp.getMods() == MODS.IMPORT) {
					...
					it.addActionListener(_ -> {
						...
						if (res == JFileChooser.APPROVE_OPTION) {
							...
							AbstractObservableWorker<List<MTGCardStock>, MTGCard, MTGCardsExport> sw = new AbstractObservableWorker<>(
									lblLoading, exp, total) {
								@Override
								...
								protected void notifyEnd() {
									MTGControler.getInstance()
											.notify(new MTGNotification(
													MTGControler.getInstance().getLangService().combine("IMPORT",
															"FINISHED"),
													exp.getName() + " " + capitalize("FINISHED"), MESSAGE_TYPE.INFO));
								}
		...
		btnApplyModification.addActionListener(_ -> {
			int res = JOptionPane.showConfirmDialog(null, capitalize("CHANGE_X_ITEMS", table.getSelectedRowCount()),
					capitalize("CONFIRMATION"), JOptionPane.YES_NO_CANCEL_OPTION);
			if (res == JOptionPane.YES_OPTION) {
	...
	private void initGUI() {
		...
		btnshowMassPanel = UITools.createBindableJButton(null, MTGConstants.ICON_MANUAL, KeyEvent.VK_M,
				"stock mass panel show");
		...
		btnGeneratePrice = UITools.createBindableJButton(null, MTGConstants.ICON_EURO, KeyEvent.VK_E,
		...
		btnApplyModification = UITools.createBindableJButton(capitalize("APPLY"), MTGConstants.ICON_CHECK,
				KeyEvent.VK_A, "stock apply");
	...
	private void initRightPanel() {
		...
		rightPanel.add(new JLangLabel("SELECT", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 1));
		rightPanel.add(new JLangLabel("QTY", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 2));
		rightPanel.add(new JLangLabel("CARD_LANGUAGE", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 3));
		rightPanel.add(new JLangLabel("FOIL", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 4));
		rightPanel.add(new JLangLabel("SIGNED", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 5));
		rightPanel.add(new JLangLabel("ALTERED", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 6));
		rightPanel.add(new JLangLabel("DIGITAL", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 7));
		rightPanel.add(new JLangLabel("QUALITY", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 8));
		rightPanel.add(new JLangLabel("COLLECTION", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 9));
		rightPanel.add(new JLangLabel("COMMENT", true),
				UITools.createGridBagConstraints(null, GridBagConstraints.EAST, 0, 10));
	...
	private SwingWorker<?, ?> newLoadWorker() {
		return new AbstractObservableWorker<List<MTGCardStock>, MTGCardStock, MTGDao>(lblLoading,
	...
	public void updateCount(List<MTGCardStock> selection) {
		if (selection == null)
			lblCount.setText(
					capitalize("ITEMS_IN_STOCK") + ": " + model.getItems().stream().mapToLong(mcs -> mcs.getQte()).sum()
							+ "( distinct cards :" + model.getRowCount() + ")  / "
							+ UITools.formatDouble(model.getItems().stream().mapToDouble(
									mcs -> mcs.getValue().doubleValue() * (mcs.getQte() == 0 ? 1 : mcs.getQte())).sum())
							+ " " + MTGControler.getInstance().getCurrencyService().getCurrentCurrency());
		else
			lblCount.setText(
					capitalize("ITEMS_IN_STOCK") + ": " + model.getItems().stream().mapToLong(mcs -> mcs.getQte()).sum()
							+ "( distinct cards :" + model.getRowCount() + ")  / "
							...
							+ " " + MTGControler.getInstance().getCurrencyService().getCurrentCurrency()
							+ " selection= "
							+ UITools.formatDouble(selection.stream().mapToDouble(
									mcs -> mcs.getValue().doubleValue() * (mcs.getQte() == 0 ? 1 : mcs.getQte())).sum())
							+ MTGControler.getInstance().getCurrencyService().getCurrentCurrency());
	}
