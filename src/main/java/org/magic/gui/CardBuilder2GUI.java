...
public class CardBuilder2GUI extends MTGUIComponent {
	...
	public CardBuilder2GUI() {
		...
		btnRemoveCard = new JButton(MTGConstants.ICON_DELETE);
		...
		panelTableCards.setLayout(new BorderLayout());
		...
		panelTableCards.add(new JScrollPane(cardsTable), BorderLayout.CENTER);
		panelTableCards.add(UITools.createFlowCenterPanel(btnRemoveCard), BorderLayout.EAST);
		...
		btnRebuildSet.addActionListener(_ -> {
			...
			if (ed != null)
				ThreadManager.getInstance().runInEdt(new AbstractObservableWorker<Void, Void, PrivateMTGSetProvider>(
						buzySet, provider, ed.getCardCount()) {
		...
		btnRemoveCard.addActionListener(_ -> {
			...
			int res = JOptionPane.showConfirmDialog(null,
					MTGControler.getInstance().getLangService().get("CONFIRM_DELETE", mc));
		...
		btnNewCard.addActionListener(_ -> {
			...
			try {
				mc.setNumber(String
						.valueOf(provider.searchCardByEdition((MTGEdition) cboSets.getSelectedItem()).size() + 1));
		...
		btnRemoveEdition.addActionListener(_ -> {
			...
			MTGEdition ed = UITools.getTableSelection(editionsTable, 1);
			int res = JOptionPane.showConfirmDialog(null, MTG.capitalize("CONFIRM_DELETE", ed),
					MTG.capitalize("DELETE"), JOptionPane.YES_NO_OPTION);
			...
			public void mouseClicked(MouseEvent me) {
				...
				if (me.getClickCount() == 2) {
					...
					ThreadManager.getInstance()
							.runInEdt(new AbstractObservableWorker<BufferedImage, Void, MTGPictureProvider>(buzyCard,
									picturesProvider) {
								@Override
								...
								@Override
								protected void notifyEnd() {
									loadPicture(getResult(), imageThumbnail);
		...
		btnGenerateCard.addActionListener(_ -> {
			...
			ThreadManager.getInstance().runInEdt(new SwingWorker<MTGCard, Void>() {
				...
				protected MTGCard doInBackground() throws Exception {
					return MTG.getEnabledPlugin(MTGIA.class).generateRandomCard(text,
							(MTGEdition) cboSets.getSelectedItem(), String.valueOf(
									provider.searchCardByEdition((MTGEdition) cboSets.getSelectedItem()).size() + 1));
				}
	...
	protected void initCard(MTGCard mc) {
		...
		ThreadManager.getInstance().runInEdt(
				new AbstractObservableWorker<BufferedImage, Void, MTGPictureProvider>(buzyCard, picturesProvider) {
					@Override
					protected BufferedImage doInBackground() throws Exception {
						return plug.getPicture(mc);
					}
					...
					@Override
					protected void notifyEnd() {
						loadPicture(getResult(), panelPictures);
						panelPictures.repaint();
					}
	...
	private void loadPicture(BufferedImage img, ImagePanel2 panel) {
		try {
			...
			panel.setImg(img, ImageTools.resize(back, img.getHeight(), img.getWidth()));
		} catch (Exception _) {
			panel.setImg(new BufferedImage(10, 10, BufferedImage.TYPE_INT_ARGB));
		}
