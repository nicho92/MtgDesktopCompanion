...
public class CardKingdomTools {
	...
	public static String getCKFormattedName(MTGCard card) {
		...
		if (card.isShowCase() && !card.getEdition().getSet().contains("Strixhaven Mystical Archive")) {
			...
			if (card.getFrameVersion().contains("1997")) // maybe using isTimeShifted() ? -->
															// AbstractCardsProvider.postTreatmentCard
			{
	...
	public static String getCKFormattedSet(MTGCard card) {
		...
		if (card.getPromotypes().contains(EnumPromoType.FNM) || card.getPromotypes().contains(EnumPromoType.RELEASE)
				|| card.getPromotypes().contains(EnumPromoType.PRERELEASE)
				|| card.getPromotypes().contains(EnumPromoType.BUYABOX)) {
			set = PROMOTIONAL;
		}

		if (card.getPromotypes().contains(EnumPromoType.STARTERDECK) && !set.contains("Eighth Edition")
				&& !set.contains("Ninth Edition") && !set.contains("Magic Origins") && !set.contains("2015 Core Set")) {
			set = PROMOTIONAL;
		}
