...
public final class MTGStockItemAdapter implements JsonDeserializer<MTGStockItem>, JsonSerializer<MTGStockItem> {
	...
	@Override
	public MTGStockItem deserialize(JsonElement elem, Type interfaceType, JsonDeserializationContext context)
			throws JsonParseException {
		try {
			return context.deserialize(elem, typeForName(EnumItems.valueOf(
					elem.getAsJsonObject().get("product").getAsJsonObject().get("typeProduct").getAsString())));
		} catch (Exception _) {
