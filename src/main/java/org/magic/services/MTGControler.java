...
public class MTGControler {
	...
	private MTGControler() {
		...
		builder = new FileBasedConfigurationBuilder<>(XMLConfiguration.class)
				.configure(params.xml().setFile(xmlConfigFile).setSchemaValidation(false).setValidating(false)
	...
	public PictureDimension getPictureProviderDimension() {
		return new PictureDimension(Integer.parseInt(get("/card-pictures-dimension/width")),
	...
	public void setProperty(Object k, Object c) {
		try {
			...
			if (k instanceof MTGPlugin) {
				path = PluginRegistry.inst().getEntryFor(k).getXpath() + "[class='" + k.getClass().getName()
						+ "']/enable";
				logger.trace(path);
			} else {
				path = k.toString();
