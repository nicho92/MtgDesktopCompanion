...
public class AnnouncesUpdaterJob implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		try {
			...
			list.stream().filter(a -> a.getEndDate().before(new Date()) && a.getStatus() == STATUS.ACTIVE).toList()
					.forEach(a -> {
			...
			list.stream().filter(a -> a.getStartDate().before(new Date()) && a.getStatus() == STATUS.SOON).toList()
					.forEach(a -> {
