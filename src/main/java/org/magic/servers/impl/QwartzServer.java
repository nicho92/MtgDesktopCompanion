...
public class QwartzServer extends AbstractMTGServer {
	...
	public QwartzServer() {
		try {
			...
			if (!getFile(ORG_QUARTZ_PLUGIN_JOB_INITIALIZER_FILE_NAMES).exists()) {
				...
				FileTools.copyURLToFile(getClass().getResource("/data/default-quartz.xml"),
						getFile(ORG_QUARTZ_PLUGIN_JOB_INITIALIZER_FILE_NAMES));
			}
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		...
		m.put("org.quartz.scheduler.instanceName",
				new MTGProperty("MTGCompanion-schedule", "the instance name of qwartz"));
		m.put("org.quartz.threadPool.threadCount",
				MTGProperty.newIntegerProperty("3", "number of thread in the pool", 1, -1));
		m.put(ORG_QUARTZ_PLUGIN_JOB_INITIALIZER_FILE_NAMES, MTGProperty
				.newFileProperty(new File(MTGConstants.DATA_DIR, "quartz-config.xml"), "path to the config file"));
		m.put("org.quartz.jobStore.class",
				new MTGProperty("org.quartz.simpl.RAMJobStore", "class implementation used for the jobs"));
		m.put("org.quartz.plugin.jobInitializer.class",
				new MTGProperty("org.quartz.plugins.xml.XMLSchedulingDataProcessorPlugin",
						"Class implementation of config file processor"));
		m.put("org.quartz.plugin.jobInitializer.failOnFileNotFound",
				MTGProperty.newBooleanProperty("true", "don't startup if any error on the jobs initialization"));
		m.put("AUTOSTART", MTGProperty.newBooleanProperty(FALSE, "Run server at startup"));
		m.put("org.quartz.plugin.jobInitializer.scanInterval",
				MTGProperty.newIntegerProperty("60", "Timeout in second where new jobs are scanned", 10, -1));
	...
	public MTGDocumentation getDocumentation() {
		try {
			return new MTGDocumentation(URI
					.create("http://www.quartz-scheduler.org/documentation/quartz-2.3.0/configuration/ConfigMain.html")
					.toURL(), FORMAT_NOTIFICATION.HTML);
