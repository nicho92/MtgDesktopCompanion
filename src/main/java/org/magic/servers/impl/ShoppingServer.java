...
public class ShoppingServer extends AbstractWebServer {
	...
	public void exportWeb(File dest) throws IOException {
		...
		FileTools.saveFile(Paths.get(dest.getAbsolutePath(), "css", "extra.css").toFile(),
				MTGControler.getInstance().getWebshopService().getWebConfig().getExtraCss());
	}
	...
	public void extraConfig() {
		var holderCss = new ServletHolder("extraCssServlet", new DefaultServlet() {
			...
			protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
				...
				try {
					response.getWriter()
							.println(MTGControler.getInstance().getWebshopService().getWebConfig().getExtraCss());
				} catch (Exception e) {
