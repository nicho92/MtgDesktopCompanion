...
public class OversightServer extends AbstractMTGServer {
		...
		tache = new TimerTask() {
			...
			public void run() {
				...
				try {
					...
					logger.debug("Filtering dayly change <{}% with array of {} results", getInt(ALERT_MIN_PERCENT),
							ret.size());
	...
	public Map<String, MTGProperty> getDefaultAttributes() {
		return Map.of("AUTOSTART", MTGProperty.newBooleanProperty(FALSE, "Run server at startup"), TIMEOUT_MINUTE,
				...
				"NOTIFIER",
				new MTGProperty("Tray,Console",
						"select the notifiers to push information. Separated by comma. See  [Notifiers](Plugins#notifier)"),
				"SORT_FILTER", new MTGProperty("DAY_PRICE_CHANGE", "Select how are sort results to return",
						ArrayUtils.toStringArray(SORT.values())));
