...
public class WikiGenerator {
	...
	private static void buildePluginsIndex() throws IOException {
		...
		for (var k : PluginRegistry.inst().entrySet().stream().map(e -> e.getValue().getType().name()).sorted()
				.toList())
		...
		PluginRegistry.inst().entrySet().stream().filter(e -> e.getValue().getType() != PLUGINS.DASHLET).forEach(e -> {
			...
			ps.forEach(p -> {
				...
				builder.append(
						"![](https://raw.githubusercontent.com/nicho92/MtgDesktopCompanion/refs/heads/master/src/main/resources/icons/plugins/"
								+ p.getName().toLowerCase().replace(" ", "%20") + ".png)")
						.append("|");

				builder.append("[").append(p.getName()).append("]").append("(").append(p.getType()).append("-")
						.append(p.getName().replace(" ", "_")).append(")").append("|");
	...
	private static void buildPluginsdirectory() {
		...
		PluginRegistry.inst().listPlugins().stream().filter(p -> p.getType() != PLUGINS.DASHLET).forEach(p -> {
			...
			temp.append("* File Location : ")
					.append(p.getConfFile().getAbsolutePath()
							.replace(SystemUtils.getUserHome().getAbsolutePath(), USER_HOME_VAR).replace('\\', '/'))
					.append("\n");
			...
			if (!p.getDefaultAttributes().isEmpty()) {
				...
				p.getDefaultAttributes().entrySet().forEach(e -> {
					...
					temp.append(e.getValue().getDefaultValue().replace(SystemUtils.getUserHome().getAbsolutePath(),
							USER_HOME_VAR + "/")).append("|");
