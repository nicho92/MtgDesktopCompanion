...
public class TransactionConverter {
	...
	public static void main(String[] args) throws SQLException, IOException {
		...
		var order = MTG.getEnabledPlugin(MTGDao.class).listOrdersByIdTransaction("936685").get(0);
		
		{
			var t = new Transaction();
			 ...
			 t.setContact(c);
						  	 try {
						  		var products = MTG.getEnabledPlugin(MTGSealedProvider.class).get(order.getEdition(), order.getType());
						  		...
						  		for(var product : products)
						  		{
						  			var stockItem = new SealedStock(product);
								 	  ...
								 	  System.out.println("\t"+stockItem.getQte() + " " + stockItem.getProduct() + " " + stockItem.getProduct().getEdition().getId() + " " +stockItem.getProduct().getExtra() + " " + stockItem.getLanguage()+ " " + stockItem.getPrice());
