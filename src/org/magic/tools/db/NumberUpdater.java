...
public class NumberUpdater {
	...
	public static void update(String ed) throws Exception {
		...
		if(MTGControler.getInstance().getEnabledProviders() instanceof MtgjsonProvider)
		{
				JsonObject jsObj = new Gson().fromJson(new FileReader(new File(MTGControler.CONF_DIR,"AllSets-x.json")), JsonObject.class);
				...
				FileWriter fw = new FileWriter(new File(MTGControler.CONF_DIR,"AllSets-x.json").getAbsolutePath());
	...
	private static List<MagicCard> updateNumber(String ed) throws Exception {
		...
		List<MagicCard> editionsCards = MTGControler.getInstance().getEnabledProviders().searchCardByCriteria("set",ed,null);
