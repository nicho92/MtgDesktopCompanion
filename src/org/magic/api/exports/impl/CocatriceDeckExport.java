...
public class CocatriceDeckExport extends AbstractCardExport{
	...
	public MagicDeck importDeck(File f) throws Exception {
	    ...
	    for(int i = 0;i<result.getLength();i++)
		{
			...
			deck.getMap().put(MagicFactory.getInstance().getEnabledProviders().searchCardByCriteria("name", name, null).get(0), qte);
		}
		...
		for(int i = 0;i<result.getLength();i++)
		{
			String name = result.item(i).getAttributes().getNamedItem("name").getTextContent();
			Integer qte = Integer.parseInt(result.item(i).getAttributes().getNamedItem("number").getTextContent());
			deck.getMapSideBoard().put(MagicFactory.getInstance().getEnabledProviders().searchCardByCriteria("name", name, null).get(0), qte);
		}
		return deck;
	}
